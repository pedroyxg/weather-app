...
// --- ORB SHADER LOGIC (ULTRA OPTIMIZED) ---
function initOrb() {
    ...
    const vert = `
        precision mediump float; /* Medium precision lebih ringan */
    ...
    function resize() {
        ...
        // --- OPTIMASI RESOLUSI: Set ke 0.15 (sangat ringan) ---
        // Karena backgroundnya di-blur 40px di CSS, gambar pecah tidak akan terlihat.
        const dpr = 0.15; 
    ...
    window.addEventListener('mousemove', (e) => {
        if (!ticking) {
            window.requestAnimationFrame(() => {
                ...
                targetHover = 1 - Math.min(dist / (maxDist * 0.8), 1);
                ticking = false;
            });
            ticking = true;
        }
...
async function loadAllData(lat, lon, locationName) { setLoading(true); try { const weatherUrl = `${WEATHER_API}?latitude=${lat}&longitude=${lon}&current_weather=true&hourly=temperature_2m,weathercode,uv_index,relativehumidity_2m,visibility&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max&timezone=auto`; const airUrl = `${AIR_API}?latitude=${lat}&longitude=${lon}&current=us_aqi`; const [weatherRes, airRes] = await Promise.all([fetch(weatherUrl), fetch(airUrl)]); const weatherData = await weatherRes.json(); const airData = await airRes.json(); if (weatherData.timezone) { currentCityTimeZone = weatherData.timezone; document.getElementById('timezone-display').innerText = currentCityTimeZone.split('/')[1].toUpperCase().replace('_', ' '); } renderUI(weatherData, airData, locationName); } catch(err) { console.error(err); } finally { setLoading(false); } }

function renderUI(wData, aData, location) {
    const current = wData.current_weather;
    const daily = wData.daily;
    const desc = getWeatherDesc(current.weathercode);
    
    // UI Updates
    ui.loc.innerText = location;
    const now = new Date();
    ui.date.innerText = now.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric', timeZone: currentCityTimeZone });
    ui.day.innerText = now.toLocaleDateString('id-ID', { weekday: 'long', timeZone: currentCityTimeZone });
    ui.temp.innerText = Math.round(current.temperature);
    ui.cond.innerText = desc.l;
    ui.high.innerText = Math.round(daily.temperature_2m_max[0]);
    ui.low.innerText = Math.round(daily.temperature_2m_min[0]);
    ui.iconLg.innerHTML = `<i data-lucide="${desc.i}" class="w-12 h-12 text-white drop-shadow-md"></i>`;
    ui.wind.innerText = current.windspeed;
    ui.windDir.innerText = degToCompass(current.winddirection);
    
    const windArrowWrapper = document.getElementById('wind-arrow-wrapper');
    if(windArrowWrapper) { windArrowWrapper.style.transform = `rotate(${current.winddirection}deg)`; }
    
    // FIX UV INDEX & HOURLY LOGIC
    const currentEpoch = new Date(current.time).getTime();
    let hourIdx = 0;
    let minDiff = Infinity;

    wData.hourly.time.forEach((t, i) => {
    ...
    // UV Index: Use Daily Max if hour is 0.0 or force daily max for presentation
    const uvMax = daily.uv_index_max[0]; 
    const uvStat = getUVStatus(uvMax);
    
    ui.uv.innerText = uvMax.toFixed(1);
    ui.uvText.innerText = "Max: " + uvStat.t;
    
    const uvBar = document.getElementById('uv-bar');
    if(uvBar) { uvBar.className = `progress-fill ${uvStat.c}`; uvBar.style.width = uvStat.w; }
    
    ui.sunrise.innerText = daily.sunrise[0].split('T')[1];
    ui.sunset.innerText = daily.sunset[0].split('T')[1];

    const aqiNow = aData.current.us_aqi;
    const aqiStat = getAQIStatus(aqiNow);
    ui.aqiVal.innerText = aqiNow;
    ui.aqiText.innerText = aqiStat.t;
    const aqiBar = document.getElementById('aqi-bar');
    if(aqiBar) { aqiBar.className = `progress-fill ${aqiStat.c}`; aqiBar.style.width = aqiStat.w; }

    const humid = wData.hourly.relativehumidity_2m[hourIdx] || 0;
    const vis = wData.hourly.visibility[hourIdx] || 0;
    ui.humid.innerText = `${humid}%`;
    ui.vis.innerText = `${(vis/1000).toFixed(1)} km`;
    
    ui.hourly.innerHTML = '';
    for(let i = hourIdx; i < hourIdx + 24; i++) {
        if(!wData.hourly.time[i]) break;
        const t = wData.hourly.time[i].split('T')[1];
        const tmp = Math.round(wData.hourly.temperature_2m[i]);
        const c = wData.hourly.weathercode[i];
        const isNow = i === hourIdx;
        const borderClass = isNow ? 'border-purple-500/50 bg-white/10 shadow-[0_0_15px_rgba(168,85,247,0.3)]' : 'border-transparent hover:border-white/10 hover:bg-white/5';
        const textClass = isNow ? 'text-white font-bold' : 'text-gray-400';
        ui.hourly.innerHTML += `<div class="snap-start min-w-[70px] flex flex-col items-center justify-center p-4 rounded-2xl transition-all cursor-default border ${borderClass}"><span class="text-xs mb-3 font-mono ${textClass}">${t}</span><i data-lucide="${getWeatherDesc(c).i}" class="w-6 h-6 text-white mb-2 drop-shadow-sm"></i><span class="text-lg font-bold text-white">${tmp}°</span></div>`;
    }
    lucide.createIcons();
