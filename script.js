...
// ==========================================
// 1. BACKGROUND ORB (ANIMASI WEBGL)
// ==========================================
function initOrb() {
    ...
    // Fragment Shader (Inti Animasi Orb)
    const frag = `
        precision mediump float;
        uniform float iTime;
    ...
    function resize() {
        ...
        const dpr = 0.2; // Resolusi rendah (Optimasi Lag)
...
// ==========================================
// 2. LOGIKA CUACA & INTERAKSI (LENGKAP)
// ==========================================
...
async function loadAllData(lat, lon, locationName) { 
    setLoading(true); 
    try { 
        // URL API: Tambahkan uv_index_max & temperature_2m_max/min
        const weatherUrl = `${WEATHER_API}?latitude=${lat}&longitude=${lon}&current_weather=true&hourly=temperature_2m,weathercode,relativehumidity_2m,visibility&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max&timezone=auto`; 
        const airUrl = `${AIR_API}?latitude=${lat}&longitude=${lon}&current=us_aqi`; 
        ...
        if (weatherData.timezone) { 
            currentCityTimeZone = weatherData.timezone; 
            document.getElementById('timezone-display').innerText = currentCityTimeZone.split('/')[1].toUpperCase().replace('_', ' '); 
        } 
        renderUI(weatherData, airData, locationName); 
...
function renderUI(wData, aData, location) {
    ...
    // --- FIX H/L: Ambil langsung dari array daily ---
...
document.addEventListener('DOMContentLoaded', () => {
    // --- OPTIMASI LAG: Hapus listener berat di mobile ---
